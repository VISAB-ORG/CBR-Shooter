...
/**
 * Dieses Skript übernimmt die Möglichkeit zum Einsammeln von Munitionskisten.
 */
    ...
    private void OnTriggerEnter(Collider other)
    {
        if (other.tag.Equals("Player"))
        {
            foreach (Player player in GameControllerScript.mPlayers)
            {
                ...
                {
                    int newAmmu = player.mEquippedWeapon.mCurrentOverallAmmu + (player.mEquippedWeapon.mMagazineSize * 2);
                    ...
                    // Setzt die Koordianten der Munitionsposition.
                    GameControllerScript.ammuPositionRaw = new Vector3(0f, 0f, 0f);
                    if (player.mCBR)
                    {
                        for (int i = 0; i < player.mPlan.GetActionCount(); i++)
                        {
                            if (player.mPlan.actions[i].GetType() == typeof(CollectItem))
                            {
                                ...
                                if (cItem.destination.Contains("ammu"))
                                {
            ...
            GameControllerScript.mWeaponCrateCollected = true;
